...
with fluid.dygraph.guard(gpu_place):
    ...
    for i in range(EPOCH):
        ...
        for j, data in enumerate(train_batch_reader()):
            x = np.array([item[0] for item in data], dtype="float32").reshape(
                (-1, 3, 224, 224)
            )
            ...
            logger.info(
                "Epoch {}, step {}: train Loss: {},lr: {:06f}".format(
                    i, j, mean_loss.numpy(), optimizer.current_step_lr()
                )
            )
        ...
        logger.info(
            "validation Loss: {}, Accuracy: {}".format(
                np.mean(losses), np.mean(accuracys)
            )
        )
        ...
        if accuracy_cur > accuracy_max:
            ...
            df.to_csv("result/result.csv", sep=",", header=False, index=False)